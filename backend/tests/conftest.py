...
async def two_tenants(owner_conn):
    ...
    try:
        ...
        await owner_conn.execute("DELETE FROM ledger_entries WHERE tenant_id = ANY($1::uuid[])", ids)
        await owner_conn.execute("DELETE FROM sale_payments WHERE tenant_id = ANY($1::uuid[])", ids)
        await owner_conn.execute("DELETE FROM sale_changes WHERE tenant_id = ANY($1::uuid[])", ids)
