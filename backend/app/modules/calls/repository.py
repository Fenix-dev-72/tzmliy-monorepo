...
async def list_calls(conn: asyncpg.Connection, responsible_user_id: UUID | None) -> list[dict]:
    rows = [row async for row in _queries.list_calls(conn, responsible_user_id=responsible_user_id)]
    return [dict(r) for r in rows]
